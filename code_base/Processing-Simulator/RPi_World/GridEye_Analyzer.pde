...
class GridEye_Analyzer {
  ...
  GridEye       gridEye;
  Client        gridEyeClient_local;  // to  receive info from the local sim
  Server        gridEyeServer_live;   // to serve live stream to Control 
  GEControl     gridEyeControl;
  ...
  ////
  //MATT-GRIDEYE
  float presence_threshold = 0.10;
  float motion_threshold = 0.25;
  ...
  GridEye_Analyzer(PApplet _parent) {
    ...
    gridEye = new GridEye(parent, local_sim_port);  // <-- send this object the port to use for serving sim data locally
    delay(500);
    gridEyeClient_local = new Client(parent, my_rpi.my_address, local_sim_port);  // to listen to local sim
    println("Created GridEyeClient for local sim with IP " + my_rpi.my_address + " on port " + local_sim_port);

    gridEyeServer_live = new Server(parent, live_server_port);   // to stream live frames
    println("Created GridEyeClient with IP " + my_rpi.my_address + " on port " + live_server_port);



    ///  open serial port --- integrated with Comm_manager now.
    /*
    print(Serial.list());
     String uartName = Serial.list()[1];   // <-- may have to modiy this depending on the com port of the board 
     print("\n\rSelected:" + uartName);
     uart = new Serial(parent, uartName, 9600);
     uart.bufferUntil('@');
     */
  ...
  void calcVectors() {
    ...
    for (int m = 0; m < motionVectors.size(); m++) {
      ...
      ellipse(cTX, cTY, 5, 5);    // mark tail
      stroke(255, 255, 0, int(255 * dpValues[int(mV.x)]) );
      strokeWeight(3); 
      line(cTX, cTY, cHX, cHY);   // draw line
    }


    // draw overall or max

    overall_X *= overall_relax;  // 0.8
    overall_Y *= overall_relax;  // 0.8
    ...
    line( canvasw/4, canvash/4, canvasw/4 + overall_X * length_multiplier, canvash /4 + overall_Y * length_multiplier );

    // draw cumulative in grey
    stroke (255, 100);
    strokeWeight(5);
    line( canvasw/4, canvash/4, canvasw/4 + ox * length_multiplier, canvash /4 + oy * length_multiplier );

    // draw rotated cumulative in red
    stroke (255, 0, 0);
    strokeWeight(5);
    float oa_rad = radians(output_angle);
    ...
    line( canvasw/4, canvash/4, canvasw/4 + ( x_comp ), canvash /4 + ( y_comp ) );
  ...
  synchronized void read_grideye_port(Serial port) {

    if (gesim) {
      if (debug) println("available: " + gridEyeClient_local.available());

      if (gridEyeClient_local.available() > 130 ) {                    // hack: will have to change if threaded so we don't get half a transmission
        String gd = gridEyeClient_local.readStringUntil('@');
      ...
      getSerialData( port.readString() );
    }
  }

  synchronized void getSerialData(String theBuf) {
    if (goodFrame) return;  // sync with frame processing so we only read new frames when ready.
    // println(" Got: " + theBuf); 

    try {
      incomingValues = float(split(theBuf, ' '));
    }
    catch(NullPointerException e) {
      incomingValues = null; // for some reason, the incoming values array is coming in as a null pointer sometimes
    }

    if (incomingValues == null || incomingValues.length < 65 || incomingValues.length > 65 ) {
      if (incomingValues != null)
        println(" +++ dropped frame +++ " + incomingValues.length);// + incomingValues.length);
      return;
    } else {
      goodFrame = true;

      if (stream) {
        // send the frame to control
        gridEyeServer_live.write(theBuf);
      }
      incomingValues = shorten(incomingValues);  // get rid of the '@'
    }
  }
  ...
  void getInterestAreas(float[] pix) {
    ...
    for (int i = 1; i < 7; i++) {
      for (int j = 1; j < 7; j++) {
        ...
        if (motionValues[index] > interestThreshold) {
          motionVectors.add(new PVector((i*8+j), (i*8+j), motionValues[index]-interestThreshold));
        }
      }
    }
  }
