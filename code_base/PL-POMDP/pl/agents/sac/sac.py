...
import gym
import time
import pl.agents.sac.core as core
from pl.agents.sac.core import MLPActorCritic, combined_shape, count_vars
...
class SAC(object):
    def __init__(self, obs_space, act_space, hidden_sizes,
                 recompute_reward_in_backup=True, combine_hc_and_pb_reward=False,
    ...
    def update(self, time_step, rew_comp, logger, done=None):
        ...
        if time_step >= self.update_after and time_step % self.update_every == 0:
            for j in range(self.update_every):
                ...
                loss_q, q_info = self._compute_loss_q(batch, rew_comp)
                loss_q.backward()
