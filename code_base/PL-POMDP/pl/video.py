...
def write_segment_to_video(segment, fname, env):
    ...
    frames = [env.render_full_obs(x) for x in segment["human_obs_traj"]]
    # Append last frame multiple times to allow human to observe it
...
def export_video(frames, fname, fps=10):
    ...
    for frame in frames:
        if raw_image:
            encoder.proc.stdin.write(np.ascontiguousarray(frame[1]))
        else:
...
class ImageEncoder(object):
    ...
    def start(self):
        self.cmdline = (
            ...
            # '-vf', 'vflip', # '-vf', 'vflip' may be needed when running on MuJoCo
            '-vcodec', 'libx264',
            '-pix_fmt', 'yuv420p',
            self.output_path
        )
